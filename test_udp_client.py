...
try:
    ...
    logger.warning(
        "Port {port} in use, falling back to ephemeral port", port=CLIENT_RECEIVE_PORT
    )
...
def heartbeat():
    ...
    while running:
        ...
        try:
            ...
            logger.debug(
                "[Client][Heartbeat] Sent HEARTBEAT {count} for source {src}",
                count=count,
                src=SOURCE,
            )
        except Exception as hb_err:
            logger.exception(
                "[Client][Heartbeat] ERROR sending heartbeat: {err}", err=hb_err
            )
        time.sleep(3)
...
logger.info(
    "[Client] Listening for inference results on port {port}...",
...
try:
    while True:
        try:
            ...
            received_count += 1
            logger.info(
                "[Client] Received result {n}: {payload}",
                n=received_count,
                payload=json.dumps(result)[:200],
            )
